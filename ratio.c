...
double ChainFraqcreateN(double x,int num) //функція для переведення дробу в ланцюговий дріб
{
	...
	{
		a = (int*) realloc(a, floor(x) * sizeof(int)); //додаємо перший коефіцієнт в масив по формулі
		...
		do //цикл для додавання інших коефіцієнтів в массив
			{
				a = (int*) realloc(a, floor(1.0/x) * sizeof(int)); //додаємо наступний коефіцієнт в массив
